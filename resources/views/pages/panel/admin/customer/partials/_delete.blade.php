<x-theme.modal modalTitle="Hapus Pelanggan" modalOpenBtnIcon="fa-regular fa-trash-can"
    ...
    <form action="{{ route('panel.admin.customers.destroy', $customer) }}" method="POST" class="mt-5">
        @csrf
        @method('DELETE')

        <p>Anda yakin akan menghapus <span class="font-medium">{{ $customer->name }}</span> dari data pelanggan?</p>
