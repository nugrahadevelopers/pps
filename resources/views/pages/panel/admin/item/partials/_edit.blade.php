<x-theme.modal modalTitle="Edit Pelanggan" modalOpenBtnIcon="fa-regular fa-pen-to-square"
    ...
    <form action="{{ route('panel.admin.items.update', $item) }}" method="POST" class="mt-5">
        @csrf
        @method('PUT')
        <div>
            <x-theme.form.input-label for="name" value="Nama" />
            <x-theme.form.text-input id="name" name="name" value="{{ old('name', $item->name) }}" type="text" />
            <x-input-error :messages="$errors->get('name')" class="mt-2" />
        </div>

        <div class="mt-4">
            <x-theme.form.input-label for="price" value="Harga" />
            <x-theme.form.text-input id="price" name="price" value="{{ old('price', $item->price) }}"
                type="number" />
        ...
        <div class="mt-4">
            ...
            <x-theme.form.text-input id="description" name="description"
